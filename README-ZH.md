# CrossPoint Reader CJK版

[English](./README.md) | **[中文](./README-ZH.md)** | [日本語](./README-JA.md)

> 基于 [daveallie/crosspoint-reader](https://github.com/daveallie/crosspoint-reader) 的 **Xteink X4** 墨水屏阅读器固件 CJK 适配版。

本项目在原版 CrossPoint Reader 的基础上进行了中文化适配，支持中文界面和中文字体渲染。

![](./docs/images/cover.jpg)
![](./docs/images/custom-font.jpg)

## ✨ CJK版新增功能

### 🌙 深色模式支持
- 完整的深色模式，反转显示颜色（黑底白字）
- 进入阅读器和章节切换时无白屏闪烁
- 兼容灰度抗锯齿渲染
- 在 设置 → 显示颜色 中切换

### 🌏 多语言界面支持 (I18n)
- 支持 **中文**、**英文**、**日语** 三种界面语言
- 可在设置中随时切换界面语言
- 所有菜单、提示、设置项均已本地化

### 📝 中文字体支持
- **外置字体加载**：从 SD 卡 `/fonts/` 目录加载自定义中文字体
- 支持 Xteink 标准 `.bin` 字体格式
- 内置 LRU 缓存优化，提升中文渲染性能
- 示例字体：
  - `SourceHanSansCN-Medium_20_20x20.bin` (思源黑体)
  - `KingHwaOldSong_38_33x39.bin` (京华老宋体)

## 📦 功能列表

继承原版所有功能，包括：

- [x] EPUB 解析与渲染 (EPUB 2 和 EPUB 3)
- [x] EPUB 图片 alt 文本显示
- [x] TXT 纯文本阅读支持
- [x] 阅读进度保存
- [x] 文件浏览器（支持嵌套文件夹）
- [ ] 自定义休眠画面（支持封面显示）
- [x] WiFi 文件上传
- [x] WiFi OTA 固件更新
- [x] 字体、布局、显示样式自定义
- [x] 屏幕旋转
- [x] Calibre 无线连接与网页库集成
- [x] 封面图片显示

详细操作说明请参阅 [用户指南](./docs/USER_GUIDE-ZH.md)。

## 📥 安装方式

### 网页刷写 (推荐)

1. 使用 USB-C 连接 Xteink X4 到电脑
2. 访问 https://xteink.dve.al/ 并点击 "Flash CrossPoint firmware"

> **提示**：恢复官方固件可访问同一网址刷写，或在 https://xteink.dve.al/debug 切换启动分区

### 手动编译

请参阅下方 [开发指南](#开发) 章节。

## 🔧 开发

### 环境要求

* **PlatformIO Core** (`pio`) 或 **VS Code + PlatformIO IDE**
* Python 3.8+
* USB-C 数据线
* Xteink X4 设备

### 获取代码

```bash
git clone --recursive https://github.com/aBER0724/crosspoint-reader-cn

# 如果已克隆但未包含子模块：
git submodule update --init --recursive
```

### 编译与刷写

连接设备后执行：

```bash
pio run --target upload
```

### 中文字体配置

1. 在 SD 卡根目录创建 `/fonts/` 文件夹
2. 放入 `.bin` 格式的中文字体文件
3. 在设置中选择 "外置字体" 选项

字体文件命名格式：`FontName_size_WxH.bin`
例如：`SourceHanSansCN-Medium_20_20x20.bin`

#### 字体转换工具

iOS 用户可以使用 [点墨 - 阅读器助手](https://apps.apple.com/jp/app/%E7%82%B9%E5%A2%A8-%E9%98%85%E8%AF%BB%E5%99%A8%E5%8A%A9%E6%89%8B/id6754073002) 来转换字体为 `.bin` 格式。

**重要提示**：若出现汉字全部显示为问号，请检查转换的 `.bin` 字体文件的字间距和行间距是否设置过大，建议都不要超过 48px。

## 📚 技术细节

### 数据缓存

首次加载书籍章节时会缓存到 SD 卡，后续加载直接读取缓存。缓存目录位于 SD 卡根目录 `.crosspoint/`：

```
.crosspoint/
├── epub_12471232/       # 每本 EPUB 对应一个子目录 (epub_<hash>)
│   ├── progress.bin     # 阅读进度
│   ├── cover.bmp        # 书籍封面
│   ├── book.bin         # 书籍元数据
│   └── sections/        # 章节数据
│       ├── 0.bin
│       ├── 1.bin
│       └── ...
└── epub_189013891/
```

删除 `.crosspoint` 目录可清除全部缓存。

更多技术细节请参阅 [文件格式文档](./docs/file-formats.md)。

## 🤝 贡献

欢迎提交 Issue 和 Pull Request！

### 贡献流程

1. Fork 本仓库
2. 创建功能分支 (`feature/your-feature`)
3. 提交修改
4. 发起 Pull Request

## 📜 致谢

- [daveallie/crosspoint-reader](https://github.com/daveallie/crosspoint-reader) - 原版 CrossPoint Reader

---

**声明**：本项目与 Xteink 或 X4 硬件制造商无关，均为社区项目。
