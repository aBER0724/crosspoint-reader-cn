...
void CrossPointWebServerActivity::onEnter() {
  ...
  xTaskCreate(&CrossPointWebServerActivity::taskTrampoline,
              "WebServerActivityTask",
              2048,              // Stack size
              this,              // Parameters
              1,                 // Priority
...
void CrossPointWebServerActivity::onExit() {
  ...
  if (isApMode) {
    ...
    Serial.printf("[%lu] [WEBACT] Disconnecting WiFi (graceful)...\n",
                  millis());
    WiFi.disconnect(
        false); // false = don't erase credentials, send disconnect frame
  }
  ...
  Serial.printf(
      "[%lu] [WEBACT] [MEM] Free heap after WiFi disconnect: %d bytes\n",
      millis(), ESP.getFreeHeap());

  // Acquire mutex before deleting task
  Serial.printf(
      "[%lu] [WEBACT] Acquiring rendering mutex before task deletion...\n",
      millis());
...
void CrossPointWebServerActivity::onNetworkModeSelected(
    const NetworkMode mode) {
  Serial.printf("[%lu] [WEBACT] Network mode selected: %s\n", millis(),
                mode == NetworkMode::JOIN_NETWORK ? "Join Network"
                                                  : "Create Hotspot");
...
void CrossPointWebServerActivity::onWifiSelectionComplete(
    const bool connected) {
  Serial.printf(
      "[%lu] [WEBACT] WifiSelectionActivity completed, connected=%d\n",
      millis(), connected);
...
void CrossPointWebServerActivity::startAccessPoint() {
  ...
  if (AP_PASSWORD && strlen(AP_PASSWORD) >= 8) {
    apStarted = WiFi.softAP(AP_SSID, AP_PASSWORD, AP_CHANNEL, false,
                            AP_MAX_CONNECTIONS);
  } else {
  ...
  char ipStr[16];
  snprintf(ipStr, sizeof(ipStr), "%d.%d.%d.%d", apIP[0], apIP[1], apIP[2],
           apIP[3]);
  connectedIP = ipStr;
  ...
  // This redirects all DNS queries to our IP, making any domain typed resolve
  // to us
...
void CrossPointWebServerActivity::startWebServer() {
  ...
  if (webServer->isRunning()) {
    ...
    // that had its own rendering task. We need to make sure our display is
    // shown.
    xSemaphoreTake(renderingMutex, portMAX_DELAY);
    render();
  ...
  if (state == WebServerActivityState::SERVER_RUNNING) {
    ...
    if (webServer && webServer->isRunning()) {
      ...
      if (lastHandleClientTime > 0 && timeSinceLastHandleClient > 100) {
        Serial.printf(
            "[%lu] [WEBACT] WARNING: %lu ms gap since last handleClient\n",
...
void CrossPointWebServerActivity::render() const {
  ...
  if (state == WebServerActivityState::SERVER_RUNNING) {
    ...
    renderer.drawCenteredText(UI_12_FONT_ID, pageHeight / 2 - 20,
                              TR(STARTING_HOTSPOT), true, EpdFontFamily::BOLD);
    renderer.displayBuffer();
  }
}

void drawQRCode(const GfxRenderer &renderer, const int x, const int y,
...
void CrossPointWebServerActivity::renderServerRunning() const {
  ...
  if (isApMode) {
    ...
    renderer.drawCenteredText(UI_10_FONT_ID, startY + LINE_SPACING,
                              ssidInfo.c_str());

    renderer.drawCenteredText(SMALL_FONT_ID, startY + LINE_SPACING * 2,
                              TR(CONNECT_WIFI_HINT));

    renderer.drawCenteredText(SMALL_FONT_ID, startY + LINE_SPACING * 3,
                              TR(SCAN_QR_WIFI_HINT));
    // Show QR code for URL
    const std::string wifiConfig =
        std::string("WIFI:S:") + connectedSSID + ";;";
    drawQRCode(renderer, (480 - 6 * 33) / 2, startY + LINE_SPACING * 4,
               wifiConfig);
    ...
    renderer.drawCenteredText(SMALL_FONT_ID, startY + LINE_SPACING * 6,
                              TR(SCAN_QR_HINT));
    drawQRCode(renderer, (480 - 6 * 33) / 2, startY + LINE_SPACING * 7,
               hostnameUrl);
  } else {
    ...
    renderer.drawCenteredText(UI_10_FONT_ID, startY + LINE_SPACING * 2,
                              webInfo.c_str(), true, EpdFontFamily::BOLD);
    ...
    // Show QR code for URL
    drawQRCode(renderer, (480 - 6 * 33) / 2, startY + LINE_SPACING * 6,
               webInfo);
    renderer.drawCenteredText(SMALL_FONT_ID, startY + LINE_SPACING * 5,
                              TR(SCAN_QR_HINT));
  }
