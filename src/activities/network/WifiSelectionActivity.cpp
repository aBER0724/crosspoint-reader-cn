...
void WifiSelectionActivity::renderConnectionFailed() const {
  ...
  // Use centralized button hints
  const auto labels = mappedInput.mapLabels(TR(BACK), TR(CONFIRM), "", "");
  GUI.drawButtonHints(renderer, labels.btn1, labels.btn2, labels.btn3, labels.btn4);
}
...
void WifiSelectionActivity::renderForgetPrompt() const {
  ...
  renderer.drawCenteredText(UI_12_FONT_ID, top - 40, TR(CONNECTION_FAILED), true, EpdFontFamily::BOLD);
  ...
  renderer.drawCenteredText(UI_10_FONT_ID, top + 40, TR(FORGET_NETWORK));
  ...
  // Draw "Cancel" button
  if (forgetPromptSelection == 0) {
    renderer.drawText(UI_10_FONT_ID, startX, buttonY, (std::string("[") + TR(CANCEL) + "]").c_str());
  } else {
    renderer.drawText(UI_10_FONT_ID, startX + 4, buttonY, TR(CANCEL));
  }
