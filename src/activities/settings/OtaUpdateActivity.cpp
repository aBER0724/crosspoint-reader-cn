...
void OtaUpdateActivity::render() {
  ...
  renderer.drawCenteredText(UI_12_FONT_ID, topInset + 15, TR(UPDATE), true, EpdFontFamily::BOLD);
  ...
  if (state == WAITING_CONFIRMATION) {
    ...
    const std::string currentVersion = std::string(TR(CURRENT_VERSION)) + CROSSPOINT_VERSION;
    renderer.drawText(UI_10_FONT_ID, 20, 250, currentVersion.c_str());
    const std::string latestVersion = std::string(TR(NEW_VERSION)) + updater.getLatestVersion();
    renderer.drawText(UI_10_FONT_ID, 20, 270, latestVersion.c_str());
