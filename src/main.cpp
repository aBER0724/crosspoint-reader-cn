...
void enterDeepSleep() {
  // Record whether we're in reader before exiting
  if (currentActivity) {
    const std::string activityName = currentActivity->getName();
    APP_STATE.wasInReader = (activityName == "Reader");
    Serial.printf(
        "[%lu] [   ] Entering sleep from activity: %s (wasInReader=%d)\n",
        millis(), activityName.c_str(), APP_STATE.wasInReader);

    // Save to Flash (NVS) - instant and reliable, no SD card delays needed!
  ...
  waitForPowerRelease();
  // Give SD card extra time to complete any pending writes
  delay(200);
  Serial.printf("[%lu] [   ] Final delay complete, entering deep sleep now\n",
                millis());
  // Enter Deep Sleep
  esp_deep_sleep_start();
}
...
void onGoHome() {
  ...
  enterNewActivity(new HomeActivity(
      renderer, mappedInputManager, onContinueReading, onGoToReaderHome,
...
void setup() {
  ...
  // Initialize i18n - load user's language preference
  I18N.loadSettings();
  ...
  // Set UI font size based on fontSize setting (SMALL=0->20px, MEDIUM=1->22px,
  // LARGE=2->24px)
  ...
  // Check wakeup reason to decide where to go
  esp_sleep_wakeup_cause_t wakeup_reason = esp_sleep_get_wakeup_cause();
  Serial.printf("[%lu] [   ] Wakeup reason: %d\n", millis(), wakeup_reason);
  Serial.printf("[%lu] [   ] Open book path: %s\n", millis(),
                APP_STATE.openEpubPath.c_str());
  Serial.printf("[%lu] [   ] Was in reader: %d\n", millis(),
                APP_STATE.wasInReader);

  // ESP_SLEEP_WAKEUP_UNDEFINED (0) = not from deep sleep (power-on/reset)
  // ESP_SLEEP_WAKEUP_GPIO (2) = woke from deep sleep via GPIO
  if (APP_STATE.wasInReader && !APP_STATE.openEpubPath.empty()) {
    // Woke up from deep sleep AND was in reader -> continue reading
    Serial.printf(
        "[%lu] [   ] Woke from deep sleep in reader, continuing reading\n",
        millis());
    ...
    // Power-on reset OR woke from home page -> go to home page
    Serial.printf(
        "[%lu] [   ] Power-on or woke from home, going to home page\n",
        millis());
    APP_STATE.wasInReader = false; // Clear flag
    onGoHome();
  }
  ...
  // Clear any button events that occurred during boot/wakeup
  // This prevents the power button release from being interpreted as a "short
  // press"
  inputManager.update();
  Serial.printf("[%lu] [   ] Cleared button events after wakeup\n", millis());
}
